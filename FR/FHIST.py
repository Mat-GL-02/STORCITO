...
def Fhist(folder_pre, folder_post, output_fhist):
    ...
    save = input("Do you want to save the reclassified map and rasters? (y/n): ").strip().lower() in ('y','yes')

    def reproyectar_a_epsg_32629(src_path):
        #Reproyects raster without saving to disk - returns array in memory
        dst_crs = "EPSG:32629"
    ...
    def calcular_dnbr(b8_may, b12_may, b8_oct, b12_oct, idx):
        ...
        # Reproyect everything in memory
        ...
        # Reclasify directly in memory
        recl = np.where(dnbr < 0.27, 0, 1).astype('int32')
        
        # Read geometries for mask
        ...
        # Create masked raster in memory using MemoryFile
    ...
    # Process every year accumulating in memory
    suma_total = None
    target_meta = None
    ...
    for i in range(min(len(pre8), len(pre12), len(post8), len(post12))):
        ...
        # Reproject if necessary and accumulate
    ...
    # Show images from data in memory
    plt.figure()
    ...
    if save:
        ...
        # Save also in output_fhist for compatibility
        try:
